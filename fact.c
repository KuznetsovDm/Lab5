...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < pnum; i++)
		result = result * factorialParts[i].result % mod;
...
void getFactorialPart(struct FactorialPart *factorialPart) {
	...
	for (unsigned long long i = factorialPart->start; i <= factorialPart->finish; i++){
	    unsigned long long t = (factorialPart->result * i) % mod;
        if (t == 0 && q == 0){
            printf("%10llu * %10llu / %10llu = 0\n", factorialPart->result, i, mod);
            q = 1;
        }

		factorialPart->result = t;
	}
		
	printf("%10llu - %10llu : %10llu\n", factorialPart->start, factorialPart->finish, factorialPart->result);
}